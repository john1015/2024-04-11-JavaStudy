 ...
 * 											 ---- 생성자내에서 호출 가능 => 한번만 호출 / 첫줄에 사용한다
 * 											 super()는 상속을 받은 경우에 사용
 * 				=> 자동 처리 => 컴파일러에 처리
 * 					1. 모든 클래스는  Onject상속을 받는다 => Object 상속을 생략할 수 있다
 ...
 * 			---------------------------------
 * 			구현이 안된메소드 : 상속받은 클래스마다 기능은 동일한데 구현이 다른 경우 => 선언
 * 			------------ 추상메소드
 * 			[접근지정어] abstract 리턴형 메소드명(매게변수 ...);
 ...
 * 			void display(){} => 오류 발생(앞에 public을 붙여야 오류해결)
 * 		}
 * 			
 * 		=====> 인터페이스 이해 => Spring
 * 							   java/Oracle/JSP 
