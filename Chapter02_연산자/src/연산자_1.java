 ...
 *  ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
 *  1) Oracle 연결
 *  2) Web → JSP → JQuery (Ajax) → 1차 팀 프로젝트
 *  3) Spring → BueJS → 2차 팀 프로젝트
 *  4) Spring-Boot → React → 3차 프로젝트(개인)
 *  5) Python : Numpy,Pandas , Matplolib
 *  6) AWS(클라우드) → 호스팅
 *  *** Full Stack : Java Script → 동적 페이지
 *  	JQuery(Ajax) , VueJS , React(Redux , React-Query)
 *  	*** TypeScript , NextJS - 시간이 남으면 할것들
 *  *** 입문과정 → 입사 (실무) (이력서 제출)
 *  
 *   * 2024-04-15
 ...
 *    		= 알파벳이나 한글로 시작한다
 *    			(알파벳은 대소문을 구분한다)
 *    		= 문자의 개수는 제한이 없다 (3~10 권장) → 어떤 데이터가 저장되어 있는지 확인
 *    		= 숫자를 사용할 수 있다 (앞에 사용 금지)
 *    		= 특수문자 사용이 가능 ( _ , $ ) → 위치는 상관 X
 *    		= 키워드 : 자바에서 사용하는 문자열(단어)
 *    		  int / double ...(빨간색) → int int; : 오류
 *    		= 공백을 사용할 수 없다. → int a b : 오류
 *    		  
 *    2. 메모리 크기 (데이터형)
 *    	 --------------------------------------------------------------------------
 *    		정수 
 *    			byte (1byte) → 네트워크 통신 , 외부 데이터 읽기(크롤링) , 파일 업로드/다운로드
 *    		 	형식)
 *    				 byte 변수명 = 값
 *    				 → 값은 -128 ~ 127까지만 사용이 가능
 *    				 → byte는 연산 처리가 되면 결과값 int 
 *     				-----------------------------------
 *     					byte + byte = int
 *     					byte a = 127
 *     					byte b = 127
 ...
 *     			*** 구분자
 *     			int a=100 → 4byte
 *     			long b=100L → L,l , 8byte
 *    			
 *    	 --------------------------------------------------------------------------
 *    		실수 : 소수점 저장
 *    			float(4byte) → 많이 사용하지는 않는다 → 구분자 : F,f 
 *    			형식) float f=10.5 ==> 오류 → float f=10.5f(F)
 *    			
 *    			double(8byte) → 실수 가장 많이 사용되는 데이터형
 *    					→ default (컴퓨터에서 실수가 있는 경우 → double)
 *    					→ 소수점 15자리수까지 인식	
 *    			**** 정수 < 실수 → 크기가 바이트 크기가 아니라 수에 대한 표현을 따짐
 *    				long < float		
 *    	 --------------------------------------------------------------------------
 ...
 *    				char a ='A' 
 *    				char b =65 → 예외 사항
 *    			   → char은 연산처리시에 정수형으로 변경이 된다 → 'A'=>65,'a'=>97,'0'=>48 
 *    	 --------------------------------------------------------------------------
 ...
 *       1) 선언 → 초기값 : 사용자로부터 값을 받은 경우 , 난수(임의로 추출)
 *       2) 선언과 동시에 초기값 : int a=10;
 *       --------------------------------------------------------------------------
 *    연산처리 
 *    피연산자 / 연산자
 *    
 ...
 *    int a=10; a++; ==> a=11
 *    int a=10; a--; ==> a=9
 *    	=부정 연산자 : true → false , false → true : boolean 만 사용이 가능
 *    	→ !
 *    	boolean b= true
 *      !b → false ==> 조건 → 로그인 , 검색어 ...
 *      
 ...
 *           int a=10
 *           += , -= , *= , /= , %= , op=
 *       삼항연산자 : 연산 대상이 3개 → if~else → 페이징 , 복잡한 소스 코딩이 있는 경우 → 웹 , 게임 프로그램 , 네트워크 프로그램
 *       		(조건)?값1:값2
 *       		→ 조건 : true → 값1
 *       		→ 조건 : false → 값2
 *       
