 ...
 *    	if(조건문){조건문 true => 수행 => 종료 , false면 다음}
 *    	else if(조건문){조건문 true => 수행 => 종료 , false면 다음}
 *    	else if(조건문){조건문 true => 수행 => 종료 , false면 다음}
 *    	else {해당조건이 없는경우 수행 , 생략 가능}
 *    
 *    --------------------------------단순화 => 선택문
 *    switch(정수 , 문자 , 문자열){
 *    	case 1:
...
public class 중첩_for_6 {
	...
	public static void main(String[] args) {
		...
		// 2. 사용자 입력 요청
		while(true) {// 반복횟수가 없다 => 정답이면 종료 => 무한 루프
			System.out.print("세자리 정수를 입력하시요 : ");
			...
			if(input<100 || input>999) {
				...
				/*
				 * continue
				 * while 문일때 continue => 조건식으로 이동
				 * for문일때 continue => 증가식으로 이동
				 * 		   ┌-----┐
				 * while(조건식){	 |
				 * 	continue ----┘
				 * }
				 ...
				 * }
				 * 
				 * break는 while,for이나 상관없이 반복문 중단
				 */
			}
			// 세자리 정수를 입력한 경우
			user[0]=input/100;
		...
		// 3. 비교 => 힌트
		// 4. 정답 여부 확인 => 정답이면 종료 / 오답이면 => 다시입력이 가능
		//					break 		 continue
	}
