 ...
 *     프로그램
 *     ------
 *     1.   데이터가 저장 => 메모리 / 파일 / 데이터ㅓ베이스(오라클)
 *                               ------------------------ 제어가 불가능
 *                          |
 *                       자바에서는 파일 / 데이터베이스는 읽은 데이터를 메모리에 저장후에 관리
 *          자바에서 사용하는 언어
 *          ------------------
 ...
 *               => 웹(파일업로드 / 파일 다운로드), application(네트워크 전송)
 *               								|		
 *               							 채팅 / 쪽지 보내기 => Socket
 *               		|
 *                  WebSocket
 *             int(4byte) => 프로그램에서 사용하는 모든 변수는 int를 인식한다
 *                            -21억 4천 ~ 21억 4천
 *             long(8byte) => 금융권 / 공기업 / 빅데이터 / AI
 *             
 *             => 오라클에서 보내준 데이터를 받은 경우
 *               ------
 *               NUMBER,NUMBER(4) => int / 14자리 , 4자리
 *           -----------------------------------------------
 *           실수
 ...
 *           	EMPNO    NOT NULL NUMBER(4)     ===> int empno (사번) 
				...
				JOB               VARCHAR2(9)   ===> Sring job (직위)
				MGR               NUMBER(4)     ===> int mgr (사수번호)
				HIREDATE          DATE          ===> Date hiredate (입사일)
				SAL               NUMBER(7,2)   ===> double sal (급여)
				COMM              NUMBER(7,2)   ===> double comm (성과금)
				DEPTNO            NUMBER(2)     ===> int deptno (부서번호)
				=========================================================
				  숫자형 (NUMBER => int , double)
				  문자형 (CAHR / CARCHAR2 / CLOB => String)
				         1~2000  1~4000 4rlrk
				         ** 한글 한글자당 3byte
				  날짜형 (DATE => java.util.*)
				  2) 웹에 출력된 데이터 : 데이터 추출 (데이터형ㅇ)
				  
				  ** 변수 / 클래스 / 제어 => 난발함녀 프로글램을 짜기 어려움
				     => 한개의 이름으로 재어 => 모듈  
		     ...
		     *** 클래스 10게 => 컬렉션 : 배열의 단점을 보완
		     ...
		     공유변수 (static)
		     -------------------- Stack -----------------------------------------------
		      => 메모리 자체 관리 => {}이 끝나면 자동 회수
		      지역변수 / 매개변수
		     -------------------- Heap ------------------------------------------------
		      => 인스턴스 변수 => new 
		      => 클래스 / 배열
		      => 프로그래머 관리 => 메모리 해제(객체가 null인 경우 , 사용하지 않는 변수) => GC
		         new => delete
		         malloc() => free()
		  ...
		  => 사용자 요청에 따라서 제어
		     ...
		     1) 연산자
		        ...
		        ---------------                             ↓ => 구체적 => 구현
		        |		|	 ...
		      포유류	  양서류 	                                상속도 => 확장해서 기능변경 , 기능추가 해서 사용
		      ----                                          => extends
     ...
     홍길동   심청이
		         ...
		         이항연산자 => 현변환(자동)
		            *** 프로그램언어는 연산시에 같은 데이터형만 연산이 가능
		            ...
		            => int 변경후에 연산
		             ...
		             = 산술연산자 (+ , - , * , / , %)
		               + : 산술연산 , 문자열 결합
		               / : 0으로 나눌 수 없다 , 정수/정수 => 정수 (5/2 => 2)
		                                                    --------- 오라클 5/2 => 2.5 / 자바스크립트 5/2 => 2.5 / 파이썬도 동일 / 자바,c는 2
		               % : 남는 것의 왼쪽부호를 따라간다 
		             ...
		             = 논리연산자 ( && , || ) : boolean
		               => 두개의 조건이 있는 경우 처리
		                 && => 두개의 조건이 true인 경우에만 true
		                 || => 둘중에 한개의 조건이 true이면 true
		                 => 효율적인 연산
		                    (조건) && (조건)
		                    ----
		                    flase ===============> 뒤에 조건은 연산하지 않는다
		         ...
		         예)
		            <%
		               ...
		               회사 => 연봉의 5배 
		               => 자바 + HTML => JSP => 작은 사이트 제작 => 중소기업 사이트 , 유치원 ... 학워 ..
		     ...
		     2) 제어문
		        ...
		        조건문 (if)
		          ...
		          => 다중 조건문 : 해당되는 한개의 조건만 수행
		              if(조건){
		              	조건 : true => 문장을 수행하고 빠져나감
		              	조건 : false => 밑에있는 조건을 검색한다
		              }else if(조건){
		              	조건 : true => 문장을 수행하고 빠져나감
		              	조건 : false => 밑에있는 조건을 검색한다
		              }else if(조건){
		              	...
		              	해당되는 조건이 없는 경우 => 수행하는 문장 => default : 생략이 가능
		              }
		              
		              ==> 조건에 맞는 모든 문장을 수행 => 단일 조건문 여러개
		              ==> 조건에 맞는 문장 한개 수행 => 다중 조건문 
		          ...
		          반복문(for,while) : 화면 출력 => HTML(JSP)
		                     ...
		                     => 2차 for
		                         ...
		                         => 코딩 테스트 (80%)
		                            ...
		                            for(초기값;조건식;증가식){ ==> 줄수
		                            		  |true↓
		                              -------------------------------
		                            		  		|false 일시 => 1차 for문의 증가식
		                            		  1		2   4
		                            	for(초기값;조건식;증가식){ ==> 한줄에 몇개
		                            				|true
		                            		실행문장 3
		                            	}
		                           	   ----------------------------------
		                            ...
		                            for => 패턴이 있는 경우에만 수행
		                                   -------- 숫자의 패턴(수열)
		                            1 2 3 4 5 6 7 8 1 3 4 => 패턴이 없어서 => for은 어렵다
		                            1 3 5 7 9 ...... 
		                            1 2 3 4 5 ......
		                            => 중복없는 난수 : 예약일
		               while : 반복 횟수가 없는 경우
		               ...
		               형식)
		                   ...
		                   => 무한루프 => 반드시 종료조건을 설정한다 : esc , 키 선택시 종료
		                   for(;;) 
		                   while(true)
		  ...
		  => 묶기 시작 => 힌개의 이름으로 제어
		       ...
		       => 배열 : 일차원 배열
		          ...
		          4. 출력 
		            => for-each : 출력 전용
		            => 값을 변경 => 일반 for(인덱스번호를 이용해서 처리) => 0번부터 시작
		            
		          => 단점 
		              고정적 => 저장개수를 한번 지정하면 변경이 어렵다
		              --------------------------------------
		              개수가 다른 경우에 처리하기가 어렵다
		     ...
		     *** 명령문을 모아서 관리 => 메소드
		     ...
		     리턴형  : 요청에 대한 처리값
		             ...
		             2) 데이터가 여러개 => ArrayList ==> SELECT
		               ------------ 목록 / 검색
		             3) 관련된 데이터 : 사용자 정의 데이터형 ==> SELECTT 에서 조건이 있는 경우
		               ------------ 상세보기
		        ...
		        3) 클래스
		           => 구성요소 => 역할
		              class ClassName {
		                 ...
		                 --------------------------------
		                  생성자 , 초기화 블록
		                  ----    -------- 없는 경우도 존재 => 생성자는 없는 경우에 자동으로 default 생성자를 추가
		                    | 멤버변수의 초기화 담당
		                      -----------------
 ...
 *       int[] arr = new int[10];
 *       int i=0; => 입력
 *       int o = arr.length-1; ==> 출력 증감식for이용
 *       
 *       int i=0;
 *       int o=0;
 */
...
public class 최종정리 {
	public static void main(String[] args) throws Exception{
		...
		methods[0].invoke(h4, null);// 메소드 이름없이도 호출 가능 인덱스번호는 저장된 순서(매번 다름)임 !! 순차적(위에서 아래)이 아님
		// 매개변수
