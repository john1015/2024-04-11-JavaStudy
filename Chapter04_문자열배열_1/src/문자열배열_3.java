 ...
 *  배열의 개수 => length
 *  
 * 	1. replaceAll : 정규식(regex) => 문자 패턴을 이용해서 찾아서 변경
 * 					----
 *  			   split() => GPT
 *     맛있게 맛있는 맛있고 ... 맛있*
 * 	2. subString
...
public class 문자열배열_3 {
	...
	public static void main(String[] args) {
		String[] 과일 = { "사과:apple", "배:pear", "밤:chestnut", "잣:pine nut", "호두:walnut", "도토리:acorn", "귤:tangerine",
		...
		for (String f: 과일) {
			if(f.contains(fd)) { // startsWith
				 ...
				 * 
				 */
				System.out.println("영어는 "+f.substring(fd.indexOf(":")+1));
			}
		}
		// 영문 단어중에 가장 긴 이름을 가진 단어를 찾는다
		String[] 영어과일명=new String[과일.length];
		for (int i = 0; i < 영어과일명.length; i++) {
		...
		int min=100;
		for (String ef : 영어과일명) {
			if(max<ef.length()) max=ef.length();
			if(min>ef.length()) min = ef.length();
		}
		for (String ef : 영어과일명) {
			if(ef.length()==max) System.out.println("가장 긴 단어 : "+ef);
			if(ef.length()==min) System.out.println("가장 짧은 단어 : "+ef);
		}
		// 가장 짧은 단어를 찾기
