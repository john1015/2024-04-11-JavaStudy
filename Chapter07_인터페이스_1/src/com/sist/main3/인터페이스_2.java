 ...
 *                  => 여러개 저장 => 인스턴스 변수  ------- 자바 스크립트(Vue,React)
 *                  => 한개만 저장 => 정적변수(static) => new를 이용해서 따로 저장
 *             		-----------------------      
 ...
 *            -----------------
 *            개발 구현 => 15일
 *            -----------------  재사용(상속 = 변경할 내용이 있다 , 포함 = 벼녕할 내용이 없는 경우)
 *            테스트 => 15일        .jar
 *            배포 => 1개월
 *        추상 클래스 구성요소 / 형식
 ...
 *        		  => 추상 메소드 : 상속받은 클래스마다 기능은 동일 / 구현내용이 다른 경우
 *        		----------------------------
 *        		자바 => 추상 : 공통적인
 *        		----------------------------
 *        }
 ...
 *        
  ...
  *       		=> 독립적으로 사용이 가능하다
  *             => 클래스를 묶어서 관리
  *             => 결합성이 낮은 프로그램 => 에러시에 다른 클래스에 영향이 없다
  *             => 1. 윈도우 : 이벤트 처리
  *               			----------
  *                         버튼 클릭 / 마우스 클릭 / 키보드 입력
  *                2. 컬렉션 : 데이터를 모아서 쉽게 관리 => 자료구조
  *                3. 데이터베이스 => 연결, 닫기 , SQL문장 ...
  *                4. 웹프로그램에서 => 요청 받기 , 요청 처리 , 결과값 전송
  ...
  *             {
  *             (public static void final) 데이터형 상수명 = 값
  *             --------------------------        --------- 명시적인 초기화    
  *              없는 경우에는 자동추가(생략이 가능)
  *             ---------------------------------------
  *             상수 => 변수는 존재하지 않는다
  ...
  *                       -------------   ------------
  *                         |					|
  *                         ---------------------- IP,PORT => 클래스와 클래스간의 공통으로 사용되는변수 , 메소드가 있는 경우
 */
