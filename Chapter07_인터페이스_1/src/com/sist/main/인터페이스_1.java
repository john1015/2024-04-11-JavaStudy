 ...
 * 		  => 추상클래스의 단점을 보완
 * 			 -------- 단일 상속 => 다중 상속으로 변경
 * 			 -------- 추상클래스는 구현된 메소드 / 구현인 안된 메소드 => 인터페이스(구현이안된 메소드만)
 * 		  => 역할
 ...
 * 					  ----------- 스프링
 * 		  => 요구사항 분석 =>  인터페이스
 * 		  => 아이템 선정 => 참조할 사이트(벤치마킹) => 기능 추출 / 데이타 추출
 * 												|이터페이스 | 크롤링 (사용자 저의 데이터형)
 ...
 * 			=> default void ccc(){
 * 				구현이 가능
 * 			}(public) default void ccc( 구현이 가능)
 ...
 * 				=> 관련된 클래스를 모아서 관리
 * 				=> 결합성이 낮은 프로그램
 * 					=> 클래스와 클래스를 연결 => 수정할때 에러부분 => 인터페이스는 고정 => 수정
 * 		}
 * 	3. 인터페이스의 장점
 ...
 *                               | 카페
 *    => 면접 => 98%
 *    => 클래스가 여러개(관련) => 게시판 , 갤러기 게시판 , 댓글 ... => 사이트 (게시판 => 4개)
 *    						 ------------------------ CRUD
 *    => 결합성이 낮은 프로그램
 *    => 표준화 작업
 *                              
 ...
 *    						  => 크롤링 
 *    		  2) 인터페이스 : 클래스 여려개를 묶어서 관리 => 결합성이 낮은 프로그램
