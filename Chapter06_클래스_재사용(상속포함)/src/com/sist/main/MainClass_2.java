 ...
 * 			= 정적 변수 : 컴파일러에 의해서 자동으로 저장 => 메모리 공간이 1개 (모든 클래스에서 공유)
 * 				1) 자동으로 기본 초기화
 ...
 * 				생성자는 필요시에 사용한다
 * 				** 맴버변수 , 정적변수 , 메소드 , 생성자 => 필요한 것만 골라서 사용이 가능 
 * 				   ------------------------------ 구성요소는 다 필요한 것이 아니다
 * 					=> 변수
 * 					=> 메소드
 * 					=> 변수 + 메소드
 ...
 * 				생성자
 * 				-----
 * 					여러개 사용이 가능 , 없는 경우 (자동으로 컴파일러 첨부 = 매개변수가 없는 생성자 : 디폴트 생성자)
 * 					=> 상속의 예외 조건
 * 					  ==============
 * 					=> 클래스명과 동일
 * 					=> 리턴형이 없다
 ...
 * 					  공통 메소드 : static => 웹(데이터 베이스 연동 , 한글변환 , 암호화 , 복호화)
