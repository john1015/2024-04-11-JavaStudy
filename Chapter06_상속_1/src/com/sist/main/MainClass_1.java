 ...
 * 					------------------------------------------------------------------------------
 * 					 변수(멤버변수) 
 * 						=> 저장해서 관리할 데이터를 메모리에 저장할 목록(프로그램 종료시까지 메모리 유지)
 ...
 * 
 * 					------------------------------------------------------------------------------
 * 					 생성자 : 초기화 => 배열에 값을 채워준다( 파일릭기 , 오라클 연결)
 * 							** 클래스블록은 선언만 가능
 * 							** 선언
 * 								=> 변수 선언 => int a;
 ...
 *  		설계 ==> 저장 => 활용 => 메모리해제
 *  		---		---	   ----	  --------
 *  		class   new		.		null => 주소값이 없는 상태 (가비지 컬렉션)
 *  	---------------------------------------------------------------------- 객체의 생명주기
 *  	127page ==> 접근지정어
 *  		=> 클래스와 클래스의 연결할때 사용
 *  		=> 변수 => private
 ...
 *  			----------- 0x100 --------------
 *  								-- super ---
 *  								  -- a -- 상위클래스 메모리 할당 super (상속을 내린 클래스 객체)
 *  									 0	  ==> b.a
 *  								  -------
 *  								  display() ==> b.display()
 *  								  -------
 ...
 *  				=> 두개를 받는 경우
 *  					class A{int a}
 *  					class B extends A{int b; // int a;}
 *  					class C extends B{int c; // int a; int b;}
 ...
 *  	211page ==> 상속시 생성자 
 *  				디폴트 생성자는 관계없다
 *  				매개변수가 있는 생성자가 있는 경우에는 반드시 super()를 이용해서 상위클래스의 생성자를 먼저 호출
 *  	215page ==> 키워드 (super/this)
 *  				super => 상속을 내린 클래스의 변수,메소드 호출
 *  				this => 자신의 클래스에 있는 변수 , 메소드 호출사용
 *  				------------------------------------------
 *  				this => 반드시 정리
 *  				-----
 *  				  => 생성자 , 멤버메소드에서 사용이 가능
 ...
 *   					
 *   				  }
 *   				  classB extends A{  ====> A>B (크기는 상속을 내리는 클래스가 크다)
 *   			
 *   				  }
 *   				
 *   				  => 객체 생성
 ...
 *  			=> 상위클래스 제어 : super
 *  			=> 자신클래스 제어 : this
 *  		=> 상속을 내리는 클래스는 자신 변수,메소드만 사용 가능
 *  			상속을 받은 클래스는 자신 변수,메소드,상속내린 클래스의 변수,메소드 사용이 가능
 *  
 *  		=> 객체생성
 *  			클래스명 객체명 = new 생성자()
 *  			------			   ------
 ...
 *  		A cc = new B(); // 상속이 있는경우에만 사용이 가능
 *  		 			cc.display() => B.display() => 메소드는 오버라이딩된 메소드 효율이 된다
 *  
 *  		상속이 있는 경우에 => 변수는 선언된 클래스 , 메소드는 생성자를 따라간다
 *  
 *  		다형성 : 오버로딩 / 오버라이딩
 *  				오버로딩은 같은 메소드 명으로 새로운 기능 추가 => 중복 메소드 정의
 *  				오버라이딩 같은 메소드명으로 기존의 기능을 수정 => 재정의
 *  		--------------------------------------------
 *  					오버로딩					오버라이딩
 *  		--------------------------------------------
 *  		상태			같은 클래스내에서			상속을 받는경우
 *  		--------------------------------------------
 *  		메소드명		동일						동일
 *  		--------------------------------------------
 *  		리턴형		관계없음					동일
 ...
 *  		--------------------------------------------
 *  		객체지향 (OOP)
 *  		1. 데이터 보호 => 캡슐화 => 변수는 private : 접근금지 , 사용시에 메소드를 이용할수 있게 한다
 *  																=> 읽기(getter) / 쓰기(setter)
 ...
 */
class A{
	// dufault 생성자는 상관이 없다 => 매개변수가 있는 경우에 super => 극히 드물다(라이브러리)
	public A(int a) {
...
class B extends A{
	public B(int a) {
		...
		// 상위 클래스 호풀 => 메모리에 저장 => 하위 클래스 메모리를 저장
	}
}
