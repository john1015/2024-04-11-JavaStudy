/*
 * 객체 지향 프로그램  => 자바 (CBD)
 * --------------          => 이미 필요한 기능은 조내 => 조립해서 만드는 프로그램 (레고)
 * 1. 클래스
 * 	= 변수 설정(어떤 데이터가 필요한지) : 캡슐화 (데이터보호) => getter / setter
 * 	  -------
 * 	  배열
 ...
 * 											  ------ Jsoup
 * ------------------------- 기본문법(8장) => 프로그램 개발
 * 7. 라이브러리 (자바) : 외부환경에서 지원(Jsoup , lombok)
 * 								-------------------
 * 								mvnrepository.com
 * J2SE
 * --------------------------------------------------------------
 ...
 *    	-----------------------
 *    	데이터 초기화 => 클래스 영역안에는 선언만 가능=> 구현(파일 익기 , 화면 UI,웹 연결 , 연산처라 ,제어문)
 *    	생성자 === 모든 클래스에서 반드시 한개 이상을 포함한다
 *    				=> 생략할 셩우에는 컴파일러가 자동으로 생성 : default생성자
 ...
 *    		= 여러개를 사용이 가능 => 오버로딩
 *    		= 객체 생성시 반드시 호출
 *    		= 시작과 동시에 처리해야되는 기능이 있는 경우 => 화면UI
 *    	초기화 불록 => 인스턴스 변수 => 자동으로 처리(호출한다)
 *    	{
 *    		구현이 가능
 *    		=> 자동로그인, Cookie ,Session에 등록 , 데이터베이스 연동
 *    	}
 *    	정적 블록
 *    	static{
 *    		static변수의 초기화
 ...
 *    		  ----------- 변수,메소드 => 동시에 관리 => 객체
 *    3) 객체 활용
 *    	객체명.변수명 , 객체명.변수명() ==> . (연결된 주소에 접속하는 연산자)
 *    4) 객체 소멸
 *    dsiconnnection
 ...
 *    			책상에 올려줌 => new
 *    			  |
 *    			코드연결 => 작동하게 만든다 => 생성자 => 바로 사용이 가능하게 만든다
 *    			
 ...
 *    			------------------------------ . 접근
 *    => 자바 프로그램의 순서
 *    	관련된 클래스를 모아준다 : package
 *    ------------------------------------- cliement /server => user/admin
 *    => 입출력 => java.io , java.swing , java.util
 *    package 경로명; 한번만 사용이 가능
 *    필요힌 클래스를 가지고 온다 : 사용자 정의 , 라이브러리
 *    import .... : 여러개 사용이 가능
 *    
 *    class ClassName{
 *    	
 ...
 *     2) 명령문 => 연산자/제어문 => 단점(중복, 다시 처음으로 돌아갈수 없다)
 *     			   ---------------------------------------------
 *     			  묶어서 사용 => 메소드 
 *     ----------------------------------------------- + 클래스
 *     	클래스
 ...
 *      클래스 => 기능별로 다시 묶어서 사용
 *      		----
 *      		게시판 / 회원가입 / 맛집 / 여행 ..... 패키지 => 다른 패키지면 같은 이름의 클래스를 줄수 있다
 *      										-----
 *      										| 교보서적
 *      										| 헌책방
 *      		board
