 ...
 *  	=> 명사적 => 변수
 *  	=> 동사적 => 메소드 : 점심을 먹는다 , 시험을 본다 , 게임을 한다 ...(동적)
 *  
 ...
 *  			첫번째 정수 / 두번째 정수 / 연산자
 *  
 *  	=> ***객체란 현실세계 / 비현실 세계를 특성화 시킨것이다(100%) => 5번 정도 기회
 *  								----- 추상화 ----- 단순화
 *  
 *  
 ...
 *   		= 알파벳은 시작첫문자는 대문자로 한다
 *   		= 같은 폴더에는 같은 이름의 클래스명을 사용할수없다
 *   		= 한개의 파일(.java)에 여러개의 클래스를 만들수 있다
 *   		= 클래스 형대
 *   			= 클래스형 클래스
 *   			  => 변수만 설정 => 사용자 정의 데이터형
 *   							 ~VO , ~DTO , ~Beam(JSP)
