 ...
 *      java.lang : 자바의 기본적으로 사용이 많은 클래스를 모아서 관리 => import 생략할 수 있다
 *      --------- 292page
 ...
 *        				class B{
 *        					public void display(){
 *        						A a = new A(); => 포함 클래스가 아니다(지역변수)
 *        					}
 *        				}
 *        				class C{
 *        					A a = new A(); // 포함 클래스 => 멤버일 경우만에
 *        				}
 *        			=> 제공하는 기능 (메소드)
 ...
 *                    	 class A
 *                    	 A a = new A();
 *                       System.out.println(a); => 묵시적(생략) => toString을 생략할 수 있다
 ...
 *                       -------- ProtoTypr 패턴 => 이모티콘 / 아바타
 *                    4. finalize() : 소멸자 => 객체 메모리 해제 => 자동 호출
 *                    				 => 호출이 않되면 메모리에 남아 있다 ...
 *                    				 => System.gc() : 사용하지 않는 메모리를 회수 요청
 *                    				 => 웹/애플리케이션에서는 사용라지 않는다 => 멀티미디어(동영상 , 화상채팅 , CCTV)
 *                    													------------------------------
 *                    *** 5. equals() : 객체를 비교할때
 *                    				=> 메모리 주소값(디폴트)
 *                    				=> 멤버변수값을 비교 (오버라이딩)
...
class Food{
	...
	public Food(int fno , String name) {
		...
		// 지역변수 우선순위
		// 변수 => 지역변수,매개변수 => 멤버변수
		// 변수명이 같은 경우에는 반드시 구분 => this : 클래스 자신이 가지고 있는 객체 , super : 상속내린 클래스의 객체
		/*
		 *   메모리
		 *   --------------------------
		 *   	-------- super ---------
	 ...
	 * }
	 * 
	 * A a = new A(); a,b,display() => 사용가능
	 * B b = new B(); a,b,displau(),c,bbb()
	 * A c = new B(); a,b,display()(B가 가지고있는)
	 */
...
class Student {
	...
	// hashCode 변경 => 라이브러리는 필요한 경우에 대부분 오버라이딩해서 사용
	@Override
	public int hashCode() {
		// TODO Auto-generated method stub
...
class Box implements Cloneable {
	...
	// throw문장에 뒤에 붙는 예외클래스는 => RuntimeException관련은 없다 => 예외처리를 하고 사용한다
...
public class Object_1 {
	...
	public static void main(String[] args) {
		 ...
		 * System.out.println(s1); Sawon s2 = new Sawon(2,"박문수");
		...
		try {
			...
			System.out.println("box2 => "+box2);// 메모리 주소가 다른 경우 새로운 메모리가 생성
			// 제어를 따로 할때 사용
		...
		if(f1.equals(f2)) {
			...
			System.out.println("같은 업체");
		}
		else {
			System.out.println("f1 = "+f1);
			System.out.println("f2 = "+f2);
			System.out.println("다른 업체");
		}
