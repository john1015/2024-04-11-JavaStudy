...
public class 예외처리_7 {
	String msg;// null => null은 사용하는 주소가 없어서 => 메소드를 사용할 수 없다
	// NullPointException
	public static void main(String[] args) {
		
		try {
			//int a = Integer.parseInt(" 10");//정수변환 => 공백 오류
			...
			//예외처리_7 a = new 예외처리_7();
			//String s = a.msg.concat("Java");//문자열 결합 => + => 오류발샐
			//System.out.println(s);
			
			AAA a = new AAA();// 뒤를 BBB해야 오류 X
			BBB b = (BBB)a;//ClassCastException
			//형변환 반드시 생성자가 일치
		} catch (Exception ex) {
			// Exceptionn => 자바에서 지원하는 모든 오류 처리가 가능 => Throwable : Error도 출력 => 웹
			//ex.printStackTrace();
