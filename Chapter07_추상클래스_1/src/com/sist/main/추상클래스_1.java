 ...
 * 				=> 데이터베이스
 * 				   --------- 오라클 연동 / MySql연동 => 같은메소드를 구현
 * 					*** 웹 => 데이터베이스 연동
 * 							 J2EE => 웹 연결(브라우저)
 * 							 ---------------------- 기본 문법이 지금 배우는 과정
 * 				------------------------------------
 * 				=> 우지보수가 어려운 경우 => 기능설정 => 필요시마다 구현
 * 				=> 경우의 수가 많은경우 => 기능은 같다 => 구현이 다른경우 
 * 			}
 * 				
 * 		2) 인터페이스
 * 		3) 내부클래스
 ...
 * 
 * 			class A {
 * 				=> 익명의 클래스 => 상속없이 오버라이딩이 가능 => window 프로그램
 * 				B b = new B(){
 * 					public void display(){}
 * 				}
 * 			}
 ...
 * 		
 * 		4) 종단클래스 => 사용빈도는 거의 없다 => 라이브러리에 존재 => 확장을 할 수 없다(고정)
...
class 선 extends 도형{
	...
	// 추상클래스난 인터페이스 상속이 있는 경우에는 반드시 구현이 안된 메소드를 구현해서 사용
}
