 ...
 * 자바 조건문
 * ----------
 * 1. 단일 조건문 : 독립 문장 → 여러개 사용에 조건이 맞는 모든 문장을 수행한다
 * 
 * 	형식)
 * 		if(조건문){
 * 			조건이 true일때 실행하는 문장
...
public class 자바제어문_조건문_1 {
	...
	public static void main(String[] args) {
		...
		if(menu==1) {
			System.out.println("콜라(1900)을 선택하셨습니다");
			if(pay>=1900) {
				change=pay-1900;
			}
			else {
		...
		if(menu==2) {
			System.out.println("파워에이드(1500)을 선택하셨습니다");
			if(pay>=1500) {
				System.out.print("2.파워에이드(1500) ");
				change=pay-1500;
			}
			else {
		...
		if(menu==3) {
			System.out.println("커피(1200)을 선택하셨습니다");
			if(pay>=1200) {
				System.out.print("3.커피(1200) ");
				change=pay-1200;
			}
			else {
