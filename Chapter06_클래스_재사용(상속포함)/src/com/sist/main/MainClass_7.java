...
public class MainClass_7 {
	public static void main(String[] args) {
		 ...
		 * System.out.println("AAA(1),BBB(2),CCC(3),DDD(4),EEE(5):"); int no =
		 * scan.nextInt();
		 * 
		 * KKK k = null;// 상속받은 모든 클래스를 상위 클래스를 이용해서 한번 제어 // 데이터가 여러개 => [] => 배열명 //
		 * 클래스가 여러개인 경우 => 상속 => 상위 클래스를 이용해서 제어 switch(no) { case 1: k = new AAA();
		 * break; case 2: k = new BBB(); break; case 3: k = new CCC(); break; case 4: k
		 * = new DDD(); break; case 5: k = new EEE(); break; } k.display();
		 */
		/*
		 * if(no==1) { AAA a = new AAA(); a.display(); } else if(no==2) { BBB b= new
		 * BBB(); b.display(); } else if(no==3) { CCC c= new CCC(); c.display(); } else
		 * if(no==4) { DDD d= new DDD(); d.display(); } else if(no==5) { EEE e= new
		 * EEE(); e.display(); }
		 */
