 ...
 * 			=> 사용자 정의 클래스
 * 			----------------- 조합
 * 	= 배열 => 같은 데이터형 변수 여러개를 한번에 저장 (이름이 한개) => 인덱스 (0 ...)
 ...
 * 				=> 버튼 클릭
 * 
 * 		------------------------------------------------ 프로그램의 안정성 => 종효하지 않는 프로그램 => 예외처리 8장
 * 			라이브러리 => 조립 => 암기
 * 			Math => random() , ceil()
 * 			=> Class => 3~4
 * 			=> Collection
 ...
 * 					  ^	void display(double a , double d , double c)
 * 						void display(char a , int d , int b)
 * 
 * 						display(10.5,'A',10) double char int 
 * 					  해당매개변수가 없으면 가장 유사한 매개변수로 찾아간다(큰곳)
 * 		
 * 					  class A{
 * 						 public void display(){}
 * 					  }
 ...
 * 				*** 객체지향 프로그램은 클래스 1개로 제작하는것이 아니라 클래스 여러개를 만들어서 연결
 * 								  -------
 * 									클래스 => 레고 블록
 * 				
 * 
 ...
 * 				}
 * 			= 정적 메소드
 * 				public 리턴형 메소드명(매개변수 목록) => 클래스명.메소드명
 * 				{
 * 				}
 * 			= 추상 메소드
 ...
 * 
 * 
 * 				= 기능 구현 => 사용입력을 받는 경우 : 매개변수 , 요청 처리에 대한 결과 : 리턴형
 * 																			----- void
 * 																			=> 메소드 자체 처리
 * 				=> 입력값
 * 				=> 요청처리
