 ...
 *               	trim() : 좌우 공백 제거 => 사용자 입력에서 실수 ...
 *               	valueOf() : 모든 데이터형을 문자열로 변환
 *               	replace() , repalceAll() : 문자 변환
 *               	indexOf() , lastIndexOf() : 문자나 , 문자열의 위치 찾기
 *               	----------------------------------------------------
 *               => StringBuffer : 임시저장장소를 만들어서 문자열 결합시 최적화
 *               	=> append() : 문자열 결합
 *               => Math : 수학연산
 *               	=> random() : 임의 추출
 *               	=> ceil() : 올림 함수
 ...
 *            		=> 기능을 추가 : Calendar
 *            = 문자열을 분해
 *            	StringTokenizer => split() => 배열(제어하기가 어렵다)
 *            	--------------- 단어별 추출 
 ...
 *         													------------- 데이터베이스
 *         					---
 *         					| 브라우저에서 사용되는 언어 (브라우저에서 자바는 일반 텍스트) => HTML / CSS
 *         					  ML => <a> : 태크 => 사용자 정의
 *         					  HTML / XML은 오라클을 연결하는 언어가 아니다 ...(화면 출력)
 *         					| 자바는 오라클에 있는 데이터를 관리
 *         -------
 *         화면 출력 (JavaScript) => NodeJS
 *         String / ArrayList / Integer => 웹에서 사용하는 3개 클래스
 *                   
 */
// 323page => Random
// Random => 임의로 추출 => 정수
...
public class 라이브러리_1 {
	public static void main(String[] args) {
		Random r = new Random();
		//int number = r.nextInt(10); // 0~9 => 사이의 정수를 임의로 추출
