 ...
 * 			 ---- 지역변수는 접근지정어를 사용할수 없다 (메소드안에서 사용)
 ...
 * 			}
 * ---------------------------------------------------------------------------
 * protected : 같은 패키지에서만 접근이 가능 + 상속을 받은 경우 다른 패키지까지 접근 가능
 * ---------------------------------------------------------------------------
 ...
 * 
 * 			1. 접근지정어
 * 			   메소드 => 통신담당 (소프트웨어 => 메세지) => 클래스와 클래스를 연결 => 모든 클래스에서 사용이 가능
 * 						public
 * 			   옵션 => static : 클래스 전체에서 동일한 기능을 가지고 있는 경우(극히드물다 => 프로그램에서 1~2개정도)
 * 						웹) 한글변환, 암호화 , 복호화
 * 			   리턴형 => 기본형 , 배열 , 클래스
 * 									   | 정보 출력(변수여러개인 경우) => 상세보기
 * 								| 전송하는 데이터가 여러개 => 목록
 * 						 | 데이터값이 한개인 경우 => 장바구니의 개수
 * 			   메소드명 : 식별자 => 소문자 시작(가급적이면 => 한글은 사용하지 않는다)
 * 														=> 운영체제에 따라 한글 인식이 다르다
 * 			   매개변수 : 여러개 사용이 가능 => 3개이상 초과 금지(배열,클래스)
 * 						사용자가 보내는 요청값의 개수를 모르는 경우(... 가변형)
 * 			   ** 리턴형(요청에 대한 결과값)
