...
public class BoardDAO {
	...
	/*
	 * Collection Collection | ---------------------------- | | | List Set Map ==>
	 * interface => Map map=new HashMap() => Set set=new HashSet() => List list=new
	 * ArrayList() | | | ArrayList HashSet HashMap --------- --------- --------- =>
	 * 클래스 관리 / SQL문장 관리 -------- ---------- | 스프링 | MyBatis => 키,값 => 두개를 동시에 저장
	 * 키(중복이 없다), 값(중복이 가능) => Cookie / Session => 웹 실시간 채팅 (사용자 정보) => 순서는 없다 =>
	 * 데이터중복을 허용하지 않는다 => 데이터베이스에서 데이터를 모아서 저장 순서가 존재 (인덱스) 데이터 중복을 허용 비동기 방식 =>
	 * ORDER BY를 사용하지 않으면 순서가 일정하지 않는다
	 */
	...
	/*
	 * memberInsert(String a,String b,String c,String d,String e,String f,String
	 * g,String h,String i)
	 * -----------------------------------------------------------------------------
	 * --- MemberVO vo
	 */
	public void boardInsert(BoardVO vo) {
		try {
			...
			ps.executeUpdate();
			/*
			 * executeQuery() => 결과값이 있다 => SELECT executeUpdate() => 결과값이 없다 => commit() =>
	...
	public BoardVO boardDetailData(int no) {
		...
		try {
			getConnection();
			String sql = "UPDATE board SET " + "hit = hit+1 " + "WHERE no = ?"; // 조회수 증가
			ps = conn.prepareStatement(sql);
			ps.setInt(1, no);
			ps.executeUpdate();
