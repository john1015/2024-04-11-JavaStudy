 ...
 *  	형식) 
 *  		if(조건문){
 *  			조건 → 로그인된(true) 경우 → main으로 이동
 *  		}
 *  		else{
 *  			조건 → 로그인이 안된(false) 경우 → 다시 로그인 처리 
 *  		}
 *  		*** 가장 많이 사용되는 조건문
 *  = 다중 조건문 : 조건에 맞는 경우에만 1번 수행
 *  	→ 메뉴 , 화면 이동
 *  	형식)
 *  		if(조건문) { 조건이 true일 경우에 수행 → 종료 } false 일때는 다음 조건문
 *  		else if(조건문) { 조건이 true일 경우에 수행 → 종료 } false 일때는 다음 조건문
 *  		else if(조건문) { 조건이 true일 경우에 수행 → 종료 } false 일때는 다음 조건문
 ...
 * 		   = 반복횟수 지정
 * 		   = 형식)
 * 			for(①초기값; ②조건문; ④증가식){③반복 수행}
 * 		   = 순서)	
 * 			1 → 2 → 3 → 4 → 2 → 3 → 4 → ... 2이 false면 종료
 * 		2) 예:
 * 			1~10까지 출력
 * 			초기값 : 시작(어디부터 시작)
 * 			조건식 : 어디까지 증가할지 지정
...
public class 자바제어문_반복문_1 {
	...
	public static void main(String[] args) {
		...
		for (char c = 'Z'; c >= 'A'; c--) { // 반복은 한다 → 출력물이 다르다
			System.out.print(c+" ");
		}
		// 어디부터 시작 : 초기값
		// 어디까지 끝  : 조건식으로 표현
