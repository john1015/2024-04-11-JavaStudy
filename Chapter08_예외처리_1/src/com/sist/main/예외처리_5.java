...
// 실행시 예외는 RuntimeExceptiom

/*
 * 		CheckedException : 컴파일시 예외처리가 있는지 확인
 ...
 *         IOException : 입출력 =>(메모리 입출력 , 파일입출력 , 네트워크 입출력)
 *         SQLExceotion : **** 핵심 (오라클 실행 => SQL)
 *         => Spring / 데이터베이스 가능자
 *         	  ------- java/jsp/mvc
 *            -------------------- Back
 *            -------------------- Front (자바 스크립트)
	...
	public static void main(String[] args) {
		...
		/*try {
			for (int i = 0; i < 10; i++) {
				...
				System.out.println((i+1)+" :"+i/n);// 오류 발생 => for문 종료하고 => catch를 찾는다
			}
		}catch(Exception e) {
		...
		for (int i = 1; i <= 10; i++) {
			try {
				...
				System.out.println(i+" : 오류 발생"+e.getMessage());
			}
		}
	}
}
