 ...
 *    A.java ------------------- A.class ------------------- 번역된 소스를 한줄씩 읽어서 출력 : java
 *    		       javac            └ 컴퓨터만 알 수 있는 언어         └ 인터프리터 
 *                                  └ 배포
 *      |							AWS → 전체파일을 모아서(.war)
 *     프로그래머만 알 수 있는 언어
 *  
 ...
 *                                         수요 < 공급
 *     자바언어의 특징
 *     1. 객체지향 프로그램              ┌ 상속 → 확장 extends JFrame
 *           └ 유지보수 : 수정,추가  ,  재사용  ,  데이터보호- 캡슐화(은닉화)
 *           	           └다형성(***ㄴ 오버라이딩(modify),오버로딩(new))-면접볼때 중요!!!
 *     2. 자바 가상머신(JVM) : 모든 웅영체제에서 호환이 가능
 ...
 *           여러줄 /* 한줄 //
 *        2. 변수 : 메모리에 저장 → 한개의 데이터만 저장ㅇ이 가능
 *        3. 메소드 : 사용자가 요청시에 처리하는 기능
 *                  ------------ 기능을 분리하는 역할 다른 클래스와 연결
 *        4. 예약어 : 이미 자바에서 사용하는 단어(int, double)
 *        5. 초기화 : 데이터를 저장할때 → 처음으로 부여받는 값
 *        6. 상수 : 고정된 값 → 변경할 수 없다 final
 *         
 *        -----------------------------------------------------------1장
 *        
 *        1. 프로그램 
 *        데이터를저장       →         데이터처리           →          처리 결과 출력
 *         1) 한개(변경) → 변수  //    사용자 요청시 처리 → 웹/도스창/윈도우창/핸드폰
 *         2) 한개(변경없이) → 상수 //  연산자 / 제어문
 *         3) 여러개(배열,클래스)
 *         ------------------단점 (프로그램 종료시에 자동 삭제) → 메모리
 *         4) 영구적인 저장장치
 *         	 →File / RDBMS(오라클)
 *         2. 변수 / 상수
 *         변수 → 한개의 데이터를 저장한느 메모리 공간의 별칭 → 변경해서 사용
 *         		 데이터형 변수명;
 ...
 *           ---------------
 *           정수
 *           	byte → 1byte(8bit) → 0,1이 8개만 사용이 가능 → -128~127까미만 사용가능 : 이상 초과하면 오버플로우 → 오류발생
 *           	int → 4byte(32bit) → 0,1이 32개만 사용이 가능 →-21억4천 ~ 21억 4천
 *           					   → 컴퓨터가 인식하는 숫자의 기본 → default(모든 숫자는 int)
 *           	long → 8byte(64bit) → 0,1이 64개만 사용가능
 *           		   수 표현이 많이 있다 (금융권,공기업)
 *           		   빅데이터 ...
 *           실수
 *           	double → 8byte → 소수점15개이상
 *           	       → 실수형의 default → 컴퓨터에서 사용하는 모든 실수 →  double
 *           		   ex) float f=10.5; → 오류
 *           	                   ----- 컴퓨터에서 double로 인식 → 10.5f(F)
 *           문자
 *           	char → 2byte → 0~65535사용이 가능 → 각구의 언어 사용이 가능 → UniCode , 영문 , 숫자 → ASCII
 *           논리
 ...
 *            int i=100;
 *            long l=100l(L); → L는 long형 값이다
 *            char c='A'; → 문자 1개가 들어간다 → '' 오류발생 → ' ' 한칸 띄기
 *            double d=10.5d; → d는 생략이 가능
 *            boolean b=true/false → 메모리에 저장한 후에 필요시에는 변경해서 사용이 가능
 *            					   → 변수가 없는 경우에는 데이터값을 처리할 수 없다
 *
 *            정수값을 받아서 저장후에 춯혁
 *            
 *            1)초기화 명시적인 초기화 int a=10;
...
public class 자바실행_구조 {
	...
	public static void main(String[] args) {
		...
		System.out.println("입력받아 초기화된 정수값 : " +num);
