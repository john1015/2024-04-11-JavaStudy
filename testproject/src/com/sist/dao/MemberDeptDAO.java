...
import java.util.*;
import java.sql.*;
public class MemberDeptDAO {
	   private Connection conn;
	   private PreparedStatement ps;
	   private final String URL="jdbc:oracle:thin:@192.168.10.124:1521:XE";
	   private static MemberDeptDAO dao; // 싱글턴 
	   
	   // 1. 드라이버 등록 
	   public MemberDeptDAO()
	   {
		   try
		   {
	   ...
	   public static MemberDeptDAO newInstance()
	   {
		   if(dao==null)
			   dao=new MemberDeptDAO();
	   ...
	   public ArrayList<EmpVO> empListData()
	   {
		   ...
		   {
			   getConnection();
			   String sql="SELECT id,name,sex,birthday,post , addr1 , phone , email , content , regdate , admin"
					     +"FROM members m ,company c "
					     +"WHERE m.sabun=c.sabun";
